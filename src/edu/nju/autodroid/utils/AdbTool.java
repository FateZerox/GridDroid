...
public class AdbTool {
     ...
     * @param screenshotFolder apk截图文件路径
     * @return
     */
    public static boolean dumpScreenshot(String deviceSerial, String screenshotFolder,String count){
        File directory = new File(screenshotFolder);
        if (directory.exists())
            directory.delete();
        directory.mkdirs();
        CmdExecutor.execCmd(Configuration.getADBPath() + " -s " + deviceSerial +  " shell /system/bin/screencap -p /sdcard/screenshot.png");
        CmdExecutor.execCmd(Configuration.getADBPath() + " -s " + deviceSerial +  " pull /sdcard/screenshot.png "+ screenshotFolder + "/" + count + ".png");
        //CmdExecutor.execCmd(Configuration.getADBPath() + " -s " + deviceSerial +  " shell uiautomator dump /sdcard/"+ count + ".xml");
        //CmdExecutor.execCmd(Configuration.getADBPath() + " -s " + deviceSerial +  " pull /sdcard/" + count + ".xml " +screenshotFolder + "/" + count + ".xml");
        //CmdExecutor.execCmd(Configuration.getADBPath() + " -s " + deviceSerial +  " shell rm /sdcard/" + count + ".xml");
        return true;
    }
    /**
     * 向设备安装apk
    ...
    public static boolean installApk(String deviceSerial, String apkFilePath){
        ...
        if(success){
            ...
            for (String permission : permissions){
                CmdExecutor.execCmd(Configuration.getADBPath() + " -s " + deviceSerial + " shell pm grant " + packageName + " " + permission);
            }
    ...
    public static String getLaunchableAcvivity(String apkFilePath){
        ...
        String packageName = "";
        for (String line : lines){
            if(line.startsWith("launchable-activity:") && activity.length()==0){
                int l = line.indexOf("name='");
                String str = line.substring(l+6);
                int r = str.indexOf("'");
        ...
        if(activity.length() == 0){
            //没有找到activity
            //尝试用别的方法寻找
            String xml = CmdExecutor.execCmd(Configuration.getAaptPath() + " dump xmltree \"" + apkFilePath + "\" " + "AndroidManifest.xml");
            String regex = "\\s+A:[\\s*]android:name\\(\\w+\\)\\=\"android.intent.category.LAUNCHER\"";//\s+A:[\s*]android:name\(\w+\)\="android.intent.category.LAUNCHER"
            String[] ls = xml.split("\n");
            Pattern p = Pattern.compile(regex);
            int indicator_line = -1;
            ...
            boolean intent_filter_found = false;
            Pattern p_target_activity = Pattern.compile("\\s*A:\\s*android:targetActivity\\(\\w*\\)\\=\\\"([^\\\"]+)");
            Pattern p_intent_filter = Pattern.compile("\\s*E: intent-filter");
            if(indicator_line >= 0){
                for(int i=0; i<=indicator_line; i++){
                    if(intent_filter_found){
                        Matcher m = p_target_activity.matcher(ls[i]);
                        if(m.find()){
                            activity = m.group(1);
                            break;
                        }
                        ...
                        if(p_intent_filter.matcher(ls[i]).find())
                            intent_filter_found = true;
                    }
                }
            }
            else{
                Logger.logError("getLaunchableAcvivity 无法获得启动activity");
            }
        }
