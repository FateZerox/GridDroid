 ...
 */
public class GridWeightedStrategy implements IStrategy{
    ...
    protected double similarityThreshold = 0.9;

    public GridWeightedStrategy(IAndroidAgent androidAgent, int maxSteps, String startActivity, Logger actionLogger){
    ...
    @Override
    public boolean run() {
        double timeGetCurrentGl = 0;
        double alltime = 0;
        Date timeAll = new Date();
        try {
        ...
        try{
            ...
            while(currentSteps <= maxSteps){
                try {
                    ...
                    bw.write(groupTransaction.getGraph().getVertexSize());
                    bw.newLine();
                    bw.flush();
                } catch (IOException e) {
                    e.printStackTrace();
                ...
                if(action.actionType == Action.ActionType.NoMoreAction)
                    ...
                    GL<Group> gl_p = gl;
                    Date time = new Date();
                    gl = getCurrentGL();//获取当前group和layout

                    timeGetCurrentGl += (new Date().getTime()-time.getTime())/1000.0;
            ...
            System.out.println("timeSpendOnGetCurrentGL : "+timeGetCurrentGl);
        }
        catch (IOException e){
        ...
        alltime = (new Date().getTime()-timeAll.getTime())/1000.0;
        System.out.println("timeSpendOnGenerateGraph : "+alltime);
        return true;
    }
