...
public class Main_SimilarityCaluculation {
    ...
    public static void main(String[] args) throws IOException {
        File strategyFolder = new File("C:\\Users\\fatesqw\\Desktop\\1.3\\RepAndRegion\\repdroid\\strategy_output_Wandoujia");
        List<WindowGraph> graphList = getGraphsFromDir(strategyFolder);
        Date time = new Date();
        int count = 0;
        int errorCount = 0;
        BufferedWriter bw = new BufferedWriter(new FileWriter("output_sim2rep.txt"));
    ...
    static double getMaxSim(WindowVertex v1, WindowVertex v2){
        ...
        for(LayoutTree lt1 : v1.layoutTreeList){
            for(LayoutTree lt2 : v2.layoutTreeList){
                double sim = lt1.similarityWith(lt2, LayoutSimilarityAlgorithm.BFSThenEditdistane);
...
class WindowGraph
{
    ...
    public HashMap<String,Double> groupNodeCount = new HashMap<String,Double>();

    public int[] groupnodelist = new int[10];
    public double[] OnlyGroup = new double[48*2];
    public double[] GridSum = new double[48*2];//记录所有节点的grid之和
...
class WindowVertex
{
    ...
    public double[] nodelist = new double[48*2];//48*2
    public double[] nodelist_2 = new double[48];
    public double sim = 0;
